...
- If you get unexpected errors or behaviour, try to delete the `target/scala-3.7.2/amyc-assembly-1.7.jar` and retry.
...
- To find constructors and functions in the program, you have to search in the `SymbolTable` passed along with the program. To do so, use the three helper methods provided in the interpreter:
  - `isConstructor` will return whether the `Identifier` argument is a type constructor in the program
